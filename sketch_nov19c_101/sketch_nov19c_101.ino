...
/*
 * JEFF: Remove this line below and compile if you don't want the
 * light functionality. Use this file only if you have the light .h and
 * libraries available in your env
 */
#define PIXEL_SUPPORTED
#ifdef PIXEL_SUPPORTED
void
display_pixel (int i, int x, int y, int z)
{
    pixels.setPixelColor(i, pixels.Color(x, y, z));
}
#else /* PIXEL_SUPPORTED */
void
display_pixel (int i, int x, int y, int z)
{
    i = x = y = z = 0;
}
#endif /* PIXEL_SUPPORTED */

void setup() {
  pinMode(A0, INPUT);
  pixels.begin();
  for (int i = 0; i < NUMPIXELS; i++) { //set all pixels to off
    display_pixel(i, &pixel, 0, 0, 0);
  }
  pixels.show();
  Serial.begin(9600);
...
void loop() {
  switch (state) {
    ...
    case 2: //ask question about math class
      delay(500);
      for (int i = 0; i < NUMPIXELS; i++) {
        display_pixel (i, 40, 40, 70); // white purple
      }
      pixels.show();
    ...
    case 3: //ask question about helping girl at library
      delay(500);
      for (int i = 0; i < NUMPIXELS; i++) {
        display_pixel (i, 30, 30, 10); // yellow
      }
      pixels.show();
    ...
    case 4: //ask question about dropping ice cream
      delay(500);
      for (int i = 0; i < NUMPIXELS; i++) {
        display_pixel (i, 30, 50, 20); // light green
      }
      pixels.show();
    ...
    case 5://end of questioning, will show purple
      while (!canProceed) {
        for (int i = 0; i < 24; i = i + 2) {
          display_pixel (i, 100, 40, 100);
        }
        for (int i = 1; i < 24; i = i + 2) {
          display_pixel (i, 0, 0, 0);
        }
        pixels.show();
        delay(1000);
        for (int i = 0; i < 24; i = i + 2) {
  ...
  while (!canProceed) { //waiting for switch
    for (int i = 0; i < 24; i = i + 2) {//lights scrolling, waiting for response
      display_pixel (i, 30, 127, 200);
    }
    for (int i = 1; i < 24; i = i + 2) {
      display_pixel (i, 100, 200, 50);
    }
    pixels.show();
    delay(200);
    for (int i = 0; i < 24; i = i + 2) {
